...
func (s *State) addEntry(entry interfaces.IEBEntry) {
	_, ok := s.DataRequests[entry.GetHash().Fixed()]
	if ok {
...
func (s *State) GetDBHeightEBlockComplete() uint32 {
	return s.DBHeightEBlockComplete
}
